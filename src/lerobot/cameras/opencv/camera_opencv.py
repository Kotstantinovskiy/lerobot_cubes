...
class OpenCVCamera(Camera):
    ...
    def connect(self, warmup: bool = True) -> None:
        ...
        named_device = None
        if self.config.macos_device_name:
            from .macos_devices import resolve_macos_camera

            named_device = resolve_macos_camera(self.config.macos_device_name)
            self.index_or_path = named_device["index"]
            self.backend = cv2.CAP_AVFOUNDATION
            logger.info(f"Opening named camera: {named_device}")
        self.videocapture = cv2.VideoCapture(self.index_or_path, self.backend)
        if named_device is not None:
            try:
                if resolve_macos_camera(self.config.macos_device_name) != named_device:
                    raise RuntimeError("Camera inventory changed while opening; reconnect cameras and retry")
            except Exception:
        ...
        if self.config.macos_explicit_mode:
            from .macos_devices import configure_macos_camera_mode

            try:
                mode = configure_macos_camera_mode(
                    self.config.macos_device_name, self.capture_width, self.capture_height, self.fps
                )
                logger.info(f"Selected native camera mode: {mode}")
            except Exception:
                self.videocapture.release()
                self.videocapture = None
                raise
    ...
    def async_read(self, timeout_ms: float | None = None) -> NDArray[Any]:
        ...
        Args:
            timeout_ms (float | None): Maximum time in milliseconds to wait for a frame
                to become available. None uses config.read_timeout_ms (default 200ms).
        ...
        if wait_ms > 200:
            logger.warning(f"{self} fresh frame delayed by {wait_ms:.0f} ms; recording timing may be affected.")
        return frame
