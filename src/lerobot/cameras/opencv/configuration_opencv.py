...
class OpenCVCameraConfig(CameraConfig):
    ...
    Attributes:
        ...
        read_timeout_ms: Maximum wait for a fresh frame in async_read (default 200 ms).
    ...
    # Maximum wait for a fresh frame during async_read() when no override is supplied.
    read_timeout_ms: int = 200
    # Resolve by exact device name at connection time instead of trusting a saved index.
    macos_device_name: str | None = None
    # Explicitly select a supported NV12 input mode after OpenCV opens the session.
    macos_explicit_mode: bool = False

    def __post_init__(self) -> None:
        if self.macos_explicit_mode and not self.macos_device_name:
            raise ValueError("macos_explicit_mode requires macos_device_name")
        if self.macos_explicit_mode and (not self.width or not self.height or not self.fps):
            raise ValueError("macos_explicit_mode requires width, height and fps")
